...
let tag_arg (tag : string) (arg : 'a) (sexp_of_arg : 'a -> Sexp.t) : t =
  let sexp = sexp_of_arg arg in
  Tag_sexp (tag, sexp)

let create (tag : string) (value : 'a) (sexp_of_value : 'a -> Sexp.t) : t =
  tag_arg tag value sexp_of_value

let create_s (tag : string) (sexp : Sexp.t) = Tag_sexp (tag, sexp)

let to_string_hum (t : t) : string =
  let rec render = function
    | String s -> s
    | Tag_sexp (tag, sexp) -> tag ^ ": " ^ Sexp.to_string_hum sexp
    | Tag_t (tag, t) -> tag ^ ": " ^ render t
    | Of_sexp sexp -> Sexp.to_string_hum sexp
